...
export default function SvgD1(props) {
  ...
  const [snappedPosition, setSnappedPosition] = useState([0, 0]);
  ...
  const onPointerMove = useCallback(e => {
    ...
    setSnappedPosition( getSnappedValue(hit.current) );

  }, [getSnappedValue, setSnappedPosition]);
  ...
  return (
    ...
    >
      ...
      >
        {pathPoints.current.map( pt => {
          const { id, type, params } = pt || {};
          const { x = '-', y = '-' } = params || {};
