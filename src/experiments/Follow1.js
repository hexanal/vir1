...
export default function Follow1(props) {
  ...
  const [mass, MassFader] = useFaderControl({
    label: 'Mass',
    value: 1,
    min: 0,
    max: 5,
  ...
  const [friction, FrictionFader] = useFaderControl({
    label: 'Friction',
    value: 0.2,
    min: 0,
    max: 5,
  ...
  const [interpX, setInterpX] = useSmoothTransition(0, 0.2, 10);
  const [interpY, setInterpY] = useSmoothTransition(0, 0.2, 10);
  ...
  }, [getSnappedValue, setInterpX, setInterpY]);
